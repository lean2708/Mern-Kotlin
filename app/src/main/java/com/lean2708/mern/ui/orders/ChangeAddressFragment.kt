...
class ChangeAddressFragment : Fragment() {
    ...
    companion object {
        ...
        const val ARG_SELECTED_ADDRESS_ID = "selectedAddressId"
    }
    ...
    private fun setupRecyclerView() {
        addressAdapter = CheckoutAddressAdapter(onAddressSelected = { selectedAddress ->
            sendResultAndPop(selectedAddress)
        })

        // Nhận ID địa chỉ đang chọn từ CheckoutFragment (qua arguments)
        val selectedId = arguments?.getString(ARG_SELECTED_ADDRESS_ID)
        addressAdapter.selectedAddressId = selectedId
        ...
        viewModel.defaultAddress.observe(viewLifecycleOwner) { resource ->
            when (resource) {
                ...
                is Resource.Success -> {
                    ...
                    val addressList = resource.data as? List<Address> ?: emptyList()
                ...
                is Resource.Error -> {
                    ...
                    Toast.makeText(
                        requireContext(),
                        "Lỗi tải địa chỉ: ${resource.message}",
                        Toast.LENGTH_LONG
                    ).show()
                }
                else -> Unit
    ...
    private fun sendResultAndPop(address: Address) {
        parentFragmentManager.setFragmentResult(
            REQUEST_ADDRESS_KEY,
            Bundle().apply {
                putParcelable(BUNDLE_ADDRESS_KEY, address)
            }
        )
        parentFragmentManager.popBackStack()
    }
